...
export namespace WordLists {
  ...
  for (let i = 0; i < WORD_LIST.length; i++) {
    ...
    if (!arpa) {
      global.console.error(`No ARPAbet transliteration for '${WORD_LIST[i]}'`);
    }
    arpa_vowel = arpa.split(" ").filter((v) => arpa_vowels.indexOf(v) !== -1);
    hvd = arpa_to_hvd_map[arpa_vowel];
    if (!arpa) {
      global.console.error(`No HVD for ARPAbet syllable '${arpa_vowel}'`);
    }
    words[WORD_LIST[i]] = {
      display: WORD_LIST[i],
      highlight: highlightVowel(WORD_LIST[i]),
      vowel: hvd,
      description: arpa_to_description_map[arpa_vowel],
    };
  }
  export const WORDS = words;
  export const KEYWORD_LIST: string[] = [
    "knot",
    "dart",
    "knee",
    "tin",
  ...
  Object.keys(KEYWORD_EXAMPLES_LIST).map((key) =>
    KEYWORD_EXAMPLES_LIST[key].map((word) => examples.add(word))
  );
  export const EXAMPLE_LIST: string[] = Array.from(examples).sort((a, b) =>
