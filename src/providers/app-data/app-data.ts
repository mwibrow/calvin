...
export class AppDataProvider {
  ...
  getExampleWord(): Word {
    return this.exampleWords[
      this.keywordExampleMap[this.getKeyword().hvd][this.exampleWordIndex]
    ];
  }
  ...
  setUpTalkers(config) {
    ...
    this.talkers = this.talkerList.reduce((obj, talker) => {
      ...
      return Object.assign(obj, {
        [talker]: new Talker(talker, name, name, avatar),
      });
    }, {});
  }
  ...
  setUpKeywords(config) {
    ...
    this.keywords = this.keywordList.reduce(
      (obj, keyword) =>
        Object.assign(obj, { [keyword]: this.setUpWord(keyword) }),
      {}
    );
  }

  setUpExampleWords(config: any) {
    this.exampleWordList = config.words.items.filter(
      (item) => config.keywords.items.indexOf(item) === -1
    );
  ...
  setUpKeywordExampleMap() {
    ...
    this.exampleWordList.map((exampleWord) => {
      ...
      hvdMap[word.hvd] = Array.isArray(hvdMap[word.hvd])
        ? hvdMap[word.hvd].concat(exampleWord)
        : [exampleWord];
    });
  ...
  setUpWord(word) {
    let arpa: string, vowels: string[], vowel: string, hvd: string;
    arpa = beep[word];
    if (!arpa) {
      global.console.error(`No ARPAbet transliteration for '${word}'`);
    }
    vowels = arpa.split(" ").filter((v) => arpa_vowels.indexOf(v) !== -1);
    if (!vowels.length) {
      global.console.error(`No ARPAbet vowel in '${word}'`);
    }
    vowel = vowels[0];
    hvd = arpa_to_hvd_map[vowel];
    if (!hvd) {
      global.console.error(`No HVD for ARPAbet syllable '${vowel}'`);
    }
    return new Word(
      word.toLowerCase(),
      word,
  ...
  setUpKeywordGroups(config: Config) {
    ...
    Object.keys(config.keywordGroups).map((name) => {
      ...
      words = config.keywordGroups[name].map((hvd) => {
        return this.keywordList.filter(
          (keyword) => this.keywords[keyword].hvd === hvd
        )[0];
      });
