...
import {
  VocalTractGestures,
  parseVowelDescriptions,
} from "./vocal-tract-gestures";
...
export class VocalTractAnimationComponent {
  ...
  setupVocalTract() {
    ...
    this.svg
      .querySelectorAll("g")
      .forEach((g) => g.setAttribute("style", "opacity: 0;"));
    ...
    for (i = 0; i < paths.length; i++) {
      ...
      this.vocalTract[path.getAttribute("id")] = Geometry.SvgPath.fromPathNode(
        path
      );
  ...
  setUpVowelAnimation(description: string, audioUri?: string) {
    ...
    const vowels = parseVowelDescriptions(description);
    ...
    switch (vowels.length) {
      ...
      case 2:
        ...
        break;
      default:
        global.console.error(`Cannot current animate ${vowels.length} vowels`);
    }
  }

  setupMonophthong(vowel) {
    this.gestures = new VocalTractGestures(this.vocalTract);
    const tongueTarget = this.gestures.getTongueTarget(vowel.front, vowel.open);
    const n: number = Math.floor(this.rangeMax * 0.2);
    const howWide = (vowel.open + 1) / 3 + 0.25;
    ...
    this.gestures.addTongueMovement(
      0,
      n,
      this.gestures.getTongueTarget(0, 0),
      tongueTarget
    );
    this.gestures.addTongueMovement(
      n + 1,
  ...
  setupDiphthong(vowels) {
    this.gestures = new VocalTractGestures(this.vocalTract);
    let tongueTarget1 = this.gestures.getTongueTarget(
      vowels[0].front,
      vowels[0].open
    );
    let tongueTarget2 = this.gestures.getTongueTarget(
      vowels[1].front,
      vowels[1].open
    );
  ...
  playAnimation() {
    ...
    this.animationFrameRequest.requestAnimationFrame((ev) =>
